...
def calculate_expert(model):
    ...
    for i, layer in enumerate(layers):
        ...
        print(f"Processing block No.{i + 1}")
        layer_final_alpha = []
        for name in subset:
...
def allocate(seed=25, beta=2.5, target_sum=55):
    ...
    quantized_vector = exponential_scaling(distribution, target_sum, beta)
    ...
    print("top_k: ", ','.join(map(str, topkk)))
