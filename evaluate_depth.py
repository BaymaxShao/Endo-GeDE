...
from utils.utils import readlines, compute_errors, compute_errors_simcol
...
def evaluate(opt):
    ...
    if opt.visualize_depth:
        vis_dir = os.path.join(opt.load_weights_folder, "vis_depth_{}".format(opt.eval_split))
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(dataloader)):
            ...
            if opt.visualize_depth and (opt.eval_split == 'servct' or opt.eval_split == 'simcol'):
                error_map = abs(gt_depth - pred_depth) / gt_depth
                pred_error_map = render_error(error_map)
                vis_error_name = os.path.join(vis_dir, sequence + "_" + keyframe + "_" + frame_id + "_error.png")
                cv2.imwrite(vis_error_name, pred_error_map)
            if opt.visualize_depth and opt.eval_split == 'hamlyn':
                vis_pred_depth = render_depth(pred_disp)
                if not os.path.exists(os.path.join(vis_dir, sequence)):
                    vis_dir_now = os.path.join(vis_dir, sequence)
                    os.makedirs(vis_dir_now, exist_ok=True)
                vis_file_name = os.path.join(vis_dir_now, sequence + "_" + keyframe + "_" + frame_id + ".png")
