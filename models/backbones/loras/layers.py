...
class AdaLinear(nn.Linear, LoRALayer):
...
class HydraLinear(nn.Linear, LoRALayer):
    # LoRA implemented in a dense layer
    def __init__(
            self,
            in_features: int,
        ...
        if r > 0:
            ...
            for i in range(self.lora_nums):
                setattr(self, f"lora_B{i}", nn.Parameter(self.weight.new_zeros((out_features, r))))
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.weight.requires_grad = False
        self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            nn.init.kaiming_uniform_(self.lora_A, a=math.sqrt(5))
            for i in range(self.lora_nums):
                nn.init.zeros_(getattr(self, f"lora_B{i}"))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            result = F.linear(x, T(self.weight), bias=self.bias)
            if self.r > 0:
                route_weight = nn.functional.softmax(self.lora_route(x), dim=-1, dtype=torch.float32)
                for i in range(self.lora_nums):
                    result += torch.unsqueeze(route_weight[:,:,i], -1) * (self.lora_dropout(x) @ (self.lora_A).T @ (
                                getattr(self, f"lora_B{i}")).T) * self.scaling
            return result
        else:
