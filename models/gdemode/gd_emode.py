...
from models.backbones.loras import Linear as LoraLinear
from models.backbones.loras import DVLinear as DVLinear
from models.backbones.loras import AdaLinear as AdaLinear
from models.backbones.loras import HydraLinear as HDLinear
from models.backbones.mymoe import MoELinear as MoELinear
from .layers import HeadDepth
from .layers import make_only_part_as_trainable, _make_scratch, _make_fusion_block, make_only_part_as_trainable_else
...
class gd_emode(nn.Module):
    ...
    def __init__(self,
        ...
        self.backbone = {
            ...
            "base": backbones.vits.vit_base(input_size=image_shape, residual_block_indexes=residual_block_indexes,
                                            include_cls_token=include_cls_token),
        }
        self.backbone_archs = {
            "small": "vits14",
        ...
        if peft_type != "none":
            if peft_type == 'bwmole':
                num_exps, top_ks = allocation.allocate()
                print('Experts Allocated!')
            for t_layer_i, blk in enumerate(encoder.blocks):
                ...
                qkv_out_features = blk.attn.qkv.out_features
                proj_in_features = blk.attn.proj.in_features
                proj_out_features = blk.attn.proj.out_features
                ...
                if peft_type == "part-dvlora":
                    ...
                    blk.attn.qkv = LoraLinear(qkv_in_features, qkv_out_features, r=self.r)
                elif peft_type == "adalora":
                    blk.mlp.fc1 = AdaLinear(mlp_in_features, mlp_hidden_features, r=self.r, lora_alpha=self.r)
                    blk.mlp.fc2 = AdaLinear(mlp_hidden_features, mlp_out_features, r=self.r, lora_alpha=self.r)
                    blk.attn.qkv = AdaLinear(qkv_in_features, qkv_out_features, r=self.r, lora_alpha=self.r)
                elif peft_type == "hdlora":
                    blk.mlp.fc1 = HDLinear(mlp_in_features, mlp_hidden_features, r=self.r, lora_alpha=self.r)
                    blk.mlp.fc2 = HDLinear(mlp_hidden_features, mlp_out_features, r=self.r, lora_alpha=self.r)
                    blk.attn.qkv = HDLinear(qkv_in_features, qkv_out_features, r=self.r, lora_alpha=self.r)
                elif peft_type == "bwmole":
                    blk.mlp.fc1 = MoELinear(mlp_in_features, mlp_hidden_features,
                                            r=self.r, lora_alpha=self.r,
                                            num_experts=num_exps[t_layer_i], top_k=top_ks[t_layer_i])
                    blk.mlp.fc2 = MoELinear(mlp_hidden_features, mlp_out_features,
                                            r=self.r, lora_alpha=self.r,
                                            num_experts=num_exps[t_layer_i], top_k=top_ks[t_layer_i])
                    blk.attn.qkv = MoELinear(qkv_in_features, qkv_out_features,
                                              r=self.r, lora_alpha=self.r,
                                              num_experts=num_exps[t_layer_i], top_k=top_ks[t_layer_i])
        self.encoder = encoder
        self.depth_head = DPTHead(self.embedding_dim, self.depth_head_feature, use_bn,
                                  out_channels=self.depth_head_out_channel, use_clstoken=use_cls_token)
        ...
        if peft_type == 'adalora' or "hydralora" or "lora":
            make_only_part_as_trainable_else(self.encoder)
        else:
            make_only_part_as_trainable(self.encoder)

        make_only_part_as_trainable(self.depth_head)


    def forward(self, pixel_values):
        pixel_values = torch.nn.functional.interpolate(pixel_values, size=self.image_shape, mode="bilinear",
                                                       align_corners=True)
